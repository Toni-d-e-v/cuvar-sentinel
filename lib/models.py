...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, ruxcryptod):
        golist = ruxcryptod.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_ruxcryptod(ruxcryptod, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_ruxcryptod(self, ruxcryptod, rec):
        import decimal
        import dashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from ruxcryptod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from ruxcryptod, with every run
        ...
        # get/create, then sync payment amounts, etc. from ruxcryptod - ruxcryptod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(ruxcryptod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from ruxcryptod! %s" % e)
            govobj.vote_delete(ruxcryptod)
            return (govobj, None)
    ...
    def vote_delete(self, ruxcryptod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(ruxcryptod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, ruxcryptod, signal, outcome):
        import dashlib
        ...
        output = ruxcryptod.rpc_command(*vote_command)
        ...
        if voted:
            ...
            self.sync_network_vote(ruxcryptod, signal)

    def sync_network_vote(self, ruxcryptod, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = ruxcryptod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ruxcryptod_GOVOBJ_TYPES['proposal']
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ruxcryptod_GOVOBJ_TYPES['superblock']
